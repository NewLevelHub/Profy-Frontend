 ...
 */
function LineChartComponent({ points, width = 560, height = 220 }: LineChartProps) {
  const padLeft = 36;
  const padRight = 20;
  const padTop = 16;
  const padBottom = 40;
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Профиль по субтестам">
      ...
      <text x={padLeft - 8} y={yFor(1)} textAnchor="end" dominantBaseline="middle" style={{ fontSize: 10, fill: 'var(--text-secondary)' }}>100%</text>
      <text x={padLeft - 8} y={yFor(0)} textAnchor="end" dominantBaseline="middle" style={{ fontSize: 10, fill: 'var(--text-secondary)' }}>0%</text>
      ...
      {coords.map((c, i) => (
        <g key={points[i].key}>
          ...
          <text x={c.x} y={c.y - 10} textAnchor="middle" style={{ fontSize: 10, fill: 'var(--text-primary)', fontWeight: 600 }}>
            {points[i].value}/{points[i].max}
          </text>
          <text x={c.x} y={height - padBottom + 16} textAnchor="middle" style={{ fontSize: 10, fill: 'var(--text-secondary)' }}>
            {points[i].label}
          </text>
        </g>
      ))}
