...
Реализовано целиком (§1–§4). Отличия от текста ниже: `careers` типизирован
как `PsychologistReviewCareer[]`, а не `unknown[]` (бэкенд валидирует эту
форму); `ResultLoadingPage` при pending-конверте ведёт сразу на `/results`,
минуя goal-check (там нечего предлагать без отчёта); пункт «Проверка
отчётов» — в `src/shared/ui/navigation/navItems.ts`. Исходный текст плана
сохранён ниже.
